...
function createYearFromNumber(value: number): Year {
	...
	return {
		...flavour,
		plus: (yearsToAdvance) => {
			if (!Number.isInteger(yearsToAdvance) || yearsToAdvance <= 0) {
				throw new TypeError(`${yearsToAdvance} must be a positive integer`)
			}

			return createYearFromNumber(value + yearsToAdvance)
		},
